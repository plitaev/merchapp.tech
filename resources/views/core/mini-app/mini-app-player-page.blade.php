...
@section('content')
    ...
    <div class="m-2">
        <div class="flow-root pb-24 sm:pb-32">
            ...
            @if ($os == "Android")
                <a href="javascript:void(0);" onClick="Telegram.WebApp.openLink('{{env('APP_URL')}}/miniapp/player/{{$video->id}}', {try_browser: 'chrome'});" class="text-white">{{env('APP_URL')}}/miniapp/player/{{$video->id}}</a>
            @endif
