...
class BotUserPrices extends Page implements HasTable, HasForms
{
    ...
    protected string $view = 'filament.resources.bot-resource.pages.bot-users-prices';
    ...
    public static ?string $label = "Индивидуальные  цены";
    ...
    public function mount(int $bot_id, int $bot_user_id, int $id): void
    {
        ...
        if($id > 0) {
            $botUserPrices = BotUserPrices::find($this->id);
        }else {
            $botUserPrices = BotUserPrices::get();
        }
    ...
    public function table(Table $table): Table
    {
        return $table
            ...
            ->query(
                BotUserPrices::with('products')->where('bot_user_id', $this->bot_user_id)
            )
            ->columns([
            ...
            ->recordActions([
                EditAction::make()->url(fn($record) => "/admin/bots/".$this->bot_user_id."/".$record->id."/bot-users-prices")
