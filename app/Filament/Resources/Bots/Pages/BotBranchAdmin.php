...
class BotBranchAdmin extends Page implements HasForms, HasTable, HasInfolists
{
    ...
    public function mount(int $bot_id, int $id): void
    {
        ...
        if ($id > 0) {
            ...
            $data['guests_bot_branch_access_id'] = 1;
            $data['members_bot_branch_access_id'] = 1;
            $data['banneds_bot_branch_access_id'] = 1;
        }
    ...
    public function form(Schema $schema): Schema
    {
        return $schema
            ->components([
                ...
                Section::make('Акция')
                    ->description(new HtmlString('Ссылка на запуск к акции в боте: <a href="https://t.me/'.$this->bot_alias.'?start='.base64_encode($this->bot_branch_type.'|'.$this->bot_branch_hash.'|0').' style="text-decoration: underline" target="_blank">https://t.me/'.$this->bot_alias.'?start='.$this->bot_branch_hash.'</a>'))
                    ->columns([
                        'sm' => 1,
                        'md' => 1,
                        'lg' => 1,
                    ...
                    Action::make('Сохранить')
                        ->action(function () {
                            ...
                            if ($this->id > 0) {
                                ...
                                return redirect('/admin/bots/'.$this->bot_id.'/'.$this->id.'/branch-admin');
