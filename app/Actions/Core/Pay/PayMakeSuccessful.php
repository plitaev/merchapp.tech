    ...
    public function handle(string $source, int $order_number, $pay_system_payment_id, $pay_system_payment_method_id, $pay_system_comission)
    {
        ...
        foreach ($products as $product) {
            if (isset($product->external_id) && isset($product->external_api_url)) {
                ...
                $data = array('email' => $pay->bot_user->email, "bot_user_id" => $pay->bot_user_id,'product_id' => $product->id, 'pay_price_rub' => 0, 'app_url' => "https://loverse.me");
                $json = '[{"email":"'.$pay->bot_user->email.'","product_id":"'.$product->id.'","bot_user_id":"'.$pay->bot_user_id.'","pay_price_rub":"0","app_url":"https://loverse.me"}]';
