...
use App\Models\Core\MaxSendMessageErrorLog;
    ...
    public function handle($bot_user, int $bot_message_id, string $bot_message_appointment = '') {
        ...
        if ($bot_message) {
            ...
            if ($bot_message->image && $send_status == 0) {
                $A['photo'] = \Max\Bot\FileUpload\InputFile::create(env('APP_URL').'/content/'.$bot_message->image);

                try {
                    $message = $telegram->sendPhoto($A);
                    $entities = $message->caption_entities;
                } catch (\Exception $exception) {
                    MaxSendMessageErrorLog::create(['chat_id' => $bot_user->telegram_chat_id, 'bot_message_id' => $bot_message_id, 'text' => $exception]);
                }
            ...
            if ($bot_message->video && $send_status == 0) {
                $A['video'] = \Max\Bot\FileUpload\InputFile::create(env('APP_URL').'/content/'.$bot_message->video);
            ...
            if ($bot_message->audio && $send_status == 0) {
                $A['audio'] = \Max\Bot\FileUpload\InputFile::create(env('APP_URL').'/content/'.$bot_message->audio);
            ...
            if ($bot_message->custom_file && $send_status == 0) {
                ...
                $A['document'] = \Max\Bot\FileUpload\InputFile::create(env('APP_URL').'/content/'.$bot_message->custom_file, $filename.'.'.$file_ext);
