...
class BotSendUserOnDay
{
    public function handle() {
        ...
        $A = [];
        foreach ($bot_users as $bot_user) {
            $pay = Pay::where('bot_user_id', $bot_user->id)->where('status', 1)->where('product_id', 2)->get();
            if ($pay) {
                if (count($pay) > 0) {
                    $A[] = $bot_user->id;
                }
        ...
        $bot_users_with_6m = BotUser::select('id')->whereIn('id', $A)->groupBy('id')->pluck('id')->toArray();



        $bot_users = BotUser::where('date_end', '>=', date('Y-m-d', time()))->whereNotIn('id', $bot_users_with_6m)->get();
        $A = [];
        foreach ($bot_users as $bot_user) {
            $pay = Pay::where('bot_user_id', $bot_user->id)->where('status', 1)->where('product_id', 3)->get();
            if ($pay) {
                if (count($pay) > 0) {
                    $A[] = $bot_user->id;
                }
        ...
        $bot_users_1m_multiple = BotUser::select('id')->whereIn('id', $A)->groupBy('id')->get();



        $bot_user_without_6m_12m_1m_multiple = $stat->bot_user_count - count($bot_users_with_6m) - count($bot_users_with_12m) - count($bot_users_1m_multiple);
        ...
        $text = str_replace('VAR_STAT_USER_ON_DAY_1M_COUNT', $bot_user_without_6m_12m_1m_multiple, $text);
        $text = str_replace('VAR_STAT_USER_ON_DAY_1M_MULTIPLE_COUNT', count($bot_users_1m_multiple), $text);
        $text = str_replace('VAR_STAT_USER_ON_DAY_6M_COUNT', count($bot_users_with_6m), $text);
        $text = str_replace('VAR_STAT_USER_ON_DAY_12M_COUNT', count($bot_users_with_12m), $text);
        ...
        $users_td = Pay::select('bot_user_id')->where('product_id', 27)->where('status', 1)->where('created_at', '>=', '2026-02-19 10:00:00')->groupBy('bot_user_id')->pluck('bot_user_id')->toArray();
        $fulls_td = Pay::whereNot('product_id', 27)->where('status', 1)->whereIn('bot_user_id', $users_td)->where('created_at', '>=', '2026-02-19 10:00:00')->whereNot('days', 1)->count();
        $olds_for_td = Pay::select('bot_user_id')->where('status', 1)->where('created_at', '<', '2026-02-19 10:00:00')->groupBy('bot_user_id')->pluck('bot_user_id')->toArray();
        $users_td_without_olds = Pay::select('bot_user_id')->where('product_id', 27)->where('status', 1)->where('created_at', '>=', '2026-02-19 10:00:00')->whereNotIn('bot_user_id', $olds_for_td)->groupBy('bot_user_id')->pluck('bot_user_id')->toArray();
        ...
        foreach ($supergroups as $supergroup) {
            $telegramQuery->handle($supergroup->bot, 'sendMessage', ['chat_id' => $supergroup->telegram_id, 'text' => urldecode($text)]);
        }
