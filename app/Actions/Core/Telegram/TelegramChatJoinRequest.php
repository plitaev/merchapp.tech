...
class TelegramChatJoinRequest
{
    public function handle(int $bot_id, $json) {
        ...
        if (isset($json['chat_join_request']['chat']['id']) && isset($json['chat_join_request']['user_chat_id'])) {
            ...
            $first_name = (isset($json['chat_join_request']['from']['first_name'])?$json['chat_join_request']['from']['first_name']:'none');
            $last_name = (isset($json['chat_join_request']['from']['last_name'])?$json['chat_join_request']['from']['last_name']:'none');
            $username = (isset($json['chat_join_request']['from']['username'])?$json['chat_join_request']['from']['username']:'none');
            $language_code = (isset($json['chat_join_request']['from']['language_code'])?$json['chat_join_request']['from']['language_code']:'no');
            ...
            if ($bot_user) {
                ...
                if (isset($bot_user->date_end)) {
                    ...
                    if ($bot_user->date_end > date("Y-m-d H:i:s", time())) {
                        $telegramChatJoinRequestCreate->handle($bot_user, $json, 1);
                    } else {
                        $botSendMessage->handle($bot_user, 'SYS_DECLINE_CHAT_JOIN_REQUEST');
                        $telegramChatJoinRequestCreate->handle($bot_user, $json, 0);
                    }
